...
public:
  RobotController()
    : Node("control_robot_node"), loop_rate_(1) // based on loop_rate, the lower the number the robot will move longer period of time
  {
  ...
  void move()
  {
    ...
    for (int i = 1; (i < 5) && rclcpp::ok(); ++i) {
      ...
      loop_rate_.sleep(); 
      // allow the robot to turn for 4 second
    }
    ...
    for (int i = 1; (i < 5) && rclcpp::ok(); ++i) {
      publisher_->publish(twist_);
      loop_rate_.sleep();
      // allow the robot to move forward for 4 second
    }
